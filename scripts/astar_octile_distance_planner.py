...
class AStarPlanner_Octile_Distance(GreedyRewiringPlanner): # or think about inherite from dijkstra if you feel like that is more natural
	...
	L = (max(sqrt(dX*dX),sqrt(dY*dY))+(sqrt(2)-1)*min(sqrt(dX*dX),sqrt(dY*dY)))*cost# Multiplied by the terrain cost of the cell
